...
var LAST_WATCHED_FOLDER_KEY = "cross-player:last-good-watched-folder";
var REQUIRED_PLUGIN_FILES = ["manifest.json", "main.js"];
var AUDIO_EXTENSIONS = ["mp3", "wav", "ogg", "opus", "m4a", "m4b", "weba", "aac", "flac", "aif", "aiff", "caf"];
var VIDEO_EXTENSIONS = ["mp4", "webm", "ogv", "mkv", "3gp"];
var SUPPORTED_MEDIA_EXTENSIONS = [...VIDEO_EXTENSIONS, ...AUDIO_EXTENSIONS];
var CrossPlayerPlugin = class extends import_obsidian.Plugin {
  constructor() {
    ...
    this.pendingPlaybackStateKeys = /* @__PURE__ */ new Set();
  }
  getLocalStorage() {
  ...
  markPlaybackStateChanged(item, timestamp = Date.now()) {
  ...
  hasPendingPlaybackStateChange(item) {
    return this.getMediaItemKeys(item).some((key) => this.pendingPlaybackStateKeys.has(key));
  }
  findMatchingQueueItem(queue, item) {
    return (queue == null ? void 0 : queue.find((candidate) => candidate.id === item.id)) || (queue == null ? void 0 : queue.find((candidate) => candidate.path === item.path));
  }
  hasDifferentPlaybackState(a, b) {
    return Math.abs((a.position || 0) - (b.position || 0)) > 1 || a.status !== b.status || !!a.finished !== !!b.finished || !!a.countedAsConsumed !== !!b.countedAsConsumed || (a.consumedAt || "") !== (b.consumedAt || "");
  }
  copyPlaybackState(target, source) {
  ...
  async reportSyncIssue(issueKey, message) {
    if (this.lastSyncIssueKey === issueKey)
      return;
    this.lastSyncIssueKey = issueKey;
    new import_obsidian.Notice(message, 9e3);
  }
  ...
  async validatePluginSyncHealth() {
    ...
    try {
      ...
      if (!(manifest == null ? void 0 : manifest.id) || !(manifest == null ? void 0 : manifest.version)) {
        await this.reportSyncIssue(
          "invalid:manifest",
      ...
      if (!hasDataFile) {
        if (backupWatchedFolder || ((_c = (_b = (_a = this.data) == null ? void 0 : _a.queue) == null ? void 0 : _b.length) != null ? _c : 0) > 0) {
          await this.reportSyncIssue(
            "missing:data",
      ...
      if (backupWatchedFolder && !((_d = parsed.settings) == null ? void 0 : _d.watchedFolder)) {
        await this.reportSyncIssue(
          "incomplete:data",
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: () => {
        if (this.mainView)
          void this.mainView.changePlaybackSpeed(0.1);
      }
    });
    this.addCommand({
  ...
  recordConsumption(item) {
    var _a;
    if (item.countedAsConsumed)
      return;
    const day = this.getTodayStatKey();
    const bucket = ((_a = this.data.consumptionStats) == null ? void 0 : _a[day]) || {
      seconds: 0,
      completedCount: 0
    };
  ...
  revertConsumption(item) {
    if (!item.countedAsConsumed || !item.consumedAt || !this.data.consumptionStats)
      return;
  ...
  async setWatchedFolder(path) {
    ...
    if (!(folder instanceof import_obsidian.TFolder)) {
      new import_obsidian.Notice(`Folder not found: ${normalizedPath}`);
      return;
    }
  ...
  async handleRename(file, oldPath) {
    ...
    if (itemsToUpdate.length > 0) {
      for (const item of itemsToUpdate) {
        ...
        if (watchedFolder2 && !this.isPathInsideWatchedFolder(newPath, watchedFolder2)) {
  ...
  async playNextUnread() {
    ...
    let currentIndex = -1;
    if (this.mainView && this.mainView.currentItem) {
      currentIndex = this.data.queue.findIndex((i) => {
        var _a;
        return i.id === ((_a = this.mainView.currentItem) == null ? void 0 : _a.id);
      });
    }
    if (currentIndex === -1) {
...
var ConfirmCleanConsumedMediaModal = class extends import_obsidian.Modal {
  constructor(app, plugin) {
    super(app);
    this.plugin = plugin;
  }
...
var ConsumptionStatsModal = class extends import_obsidian.Modal {
  constructor(app, plugin) {
    super(app);
    this.plugin = plugin;
  }
...
var CrossPlayerListView = class extends import_obsidian.ItemView {
  ...
  captureScrollPosition() {
    const list = this.contentEl.querySelector(".cross-player-list");
    if (!list)
      return;
  ...
  updateStatsDisplay() {
    ...
    const etcEl = this.contentEl.querySelector(".cross-player-etc");
    if (etcEl) {
      etcEl.setText(`ETC: ${this.plugin.formatDuration(adjustedDuration)}`);
    }
    const sizeEl = this.contentEl.querySelector(".cross-player-size");
    if (sizeEl) {
  ...
  updateDownloadProgress(parentContainer) {
    ...
    if (!container) {
      container = (_a = this.contentEl.querySelector(".cross-player-download-container")) != null ? _a : void 0;
    }
    if (!container)
      return;
    container.empty();
    ...
    header.onclick = () => {
      ...
      (0, import_obsidian.setIcon)(toggleIcon, collapsed ? "chevron-down" : "chevron-up");
    };
    if (activeDownloads.length > 0) {
      let totalProgress = 0;
      let count = 0;
...
var CrossPlayerMainView = class extends import_obsidian.ItemView {
  constructor(leaf, plugin) {
    ...
    this.overlayCurrentTimeEl = null;
    this.overlayDurationEl = null;
    this.overlayFullscreenBtn = null;
  ...
  async onOpen() {
    ...
    container.addEventListener("keydown", (e) => {
      if (!this.videoEl)
        return;
      if (activeDocument.activeElement === this.videoEl)
        return;
  ...
  createMobileOverlay(container) {
    ...
    const isProgressWrapInteractive = () => {
      ...
      return Boolean(
        progressWrapEl && isOverlayVisible() && overlay.hasClass("is-visible")
      );
    };
    const isOverlayControlTarget = (target) => {
      if (!target || !isOverlayVisible())
        return false;
      ...
      return Boolean(
        target.closest(".cross-player-overlay-progress") || target.closest(".cross-player-overlay-progress-wrap")
      );
    };
    const handlePlayerTap = (target, event) => {
      if (!isOverlayVisible()) {
        event.preventDefault();
        event.stopPropagation();
    ...
    (0, import_obsidian.setIcon)(progressFullscreenBtn, activeDocument.fullscreenElement ? "minimize" : "maximize");
    ...
    progressFullscreenBtn.onclick = (e) => {
      ...
      window.setTimeout(() => {
        if (this.overlayFullscreenBtn) {
          (0, import_obsidian.setIcon)(this.overlayFullscreenBtn, activeDocument.fullscreenElement ? "minimize" : "maximize");
        }
        showOverlay();
      }, 80);
    };
    const seekFromValue = (value) => {
      if (!this.videoEl || !isFinite(this.videoEl.duration) || this.videoEl.duration <= 0)
        return;
      const pct = Math.min(1, Math.max(0, value / 1e3));
    ...
    const seekFromClientX = (clientX) => {
      ...
      const value = Math.round(pct * 1e3);
      progressBar.value = String(value);
      seekFromValue(value);
    };
    const stopProgressGesture = (e) => {
      e.preventDefault();
      e.stopPropagation();
    };
    ...
    this.videoEl.addEventListener("play", () => (0, import_obsidian.setIcon)(playPauseBtn, "pause"));
    this.videoEl.addEventListener("pause", () => (0, import_obsidian.setIcon)(playPauseBtn, "play"));
  ...
  shouldUseTouchOverlay() {
    return import_obsidian.Platform.isMobile || import_obsidian.Platform.isTablet;
  }
  ...
  showIdlePlaceholder() {
    ...
    if (!this.audioPlaceholderEl || !this.videoEl)
      return;
    this.contentEl.removeClass("is-media-active");
    (_a = this.videoWrapperEl) == null ? void 0 : _a.removeClass("is-video-active");
    ...
    (0, import_obsidian.setIcon)(iconEl, "play-circle");
  ...
  toggleFullscreen() {
    if (!this.videoEl)
      return;
    if (!activeDocument.fullscreenElement) {
      this.contentEl.requestFullscreen().catch((error) => {
        const message = error instanceof Error ? `${error.message} (${error.name})` : "Unknown fullscreen error";
        new import_obsidian.Notice(`Error attempting to enable full-screen mode: ${message}`);
      });
    } else {
